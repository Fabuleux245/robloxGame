...
-- Decompiled on 2025-07-21 08:42:08
...
function module_upvr.LoadData(arg1) -- Line 60
	...
	if not arg1.Parent then
		...
		elseif typeof(any_callWithRetry_result1_2) == "table" then
			for i_5, v_5 in pairs(any_callWithRetry_result1_2.Outfits) do
				if v_5.BodyColors then
					for i_6, v_6 in pairs(v_5.BodyColors) do
						v_5.BodyColors[i_6] = Color3.fromHex(v_6)
					end
				end
			end
			...
			i_5 = arg1.Name
			v_5 = "\nError: "
			Utils_upvr.pprint("[SuperBiz] Couldn't load catalog data for "..i_5..v_5..any_callWithRetry_result1_2)
		end
...
function module_upvr.SaveData(arg1, arg2) -- Line 98
	...
	if any_IsStudio_result1_upvr then
		...
		for i_3, v_3 in any_deepCopy_result1_upvw.Outfits do
			for i_4, v_4 in v_3.BodyColors do
				v_3.BodyColors[i_4] = v_4:ToHex()
			end
		end
		...
		if any_callWithRetry_result1 then
			i_3 = "[SuperBiz] Couldn't save catalog data for "
			v_3 = arg1.Name
			Utils_upvr.pprint(i_3..v_3.."\nError: "..any_callWithRetry_result1)
		end
	end
end
